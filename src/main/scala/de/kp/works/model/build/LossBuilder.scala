...
trait LossBuilder extends SpecBuilder {
  ...
  def config2Loss(loss:Config): TensorLossFunction[Float] = {
   ...
   *
   */
  private def config2BCE(params:Config): BinaryCrossEntropy[Float] = {
     ...
     * each mini-batch.
     */
    val sizeAverage = getAsBoolean(params, "sizeAverage", default = true)
   ...
   */
  private def config2CCE(params:Config): CategoricalCrossEntropy[Float] = {
    objectives.CategoricalCrossEntropy()
  }
  /**
   * Cosine Proximity
   */
  private def config2Cosine(params:Config): CosineProximity[Float] = {
    objectives.CosineProximity()
  }
  /**
   * Hinge
   */
  private def config2Hinge(params:Config): Hinge[Float] = {
   ...
   */
  private def config2KLD(params:Config): KullbackLeiblerDivergence[Float] = {
    objectives.KullbackLeiblerDivergence()
  }
  /**
   * Mean Absolute Error
   */
  private def config2MAE(params:Config): MeanAbsoluteError[Float] = {
    /*
     * Whether losses are averaged over observations
     * for each mini-batch. Default is true.
     *
   ...
   */
  private def config2MAPE(params:Config): MeanAbsolutePercentageError[Float] = {
    objectives.MeanAbsolutePercentageError()
  }
  /**
   * Mean Squared Error
   */
  private def config2MSE(params:Config): MeanSquaredError[Float] = {
   ...
   */
  private def config2MSLE(params:Config): MeanSquaredLogarithmicError[Float] = {
    objectives.MeanSquaredLogarithmicError()
  }
  /**
   * Poisson
   */
  private def config2Poisson(params:Config): Poisson[Float] = {
    objectives.Poisson()
  }
  /**
   * RankHinge
   */
  private def config2RankHinge(params:Config): RankHinge[Float] = {
   ...
   */
  private def config2SCCE(params:Config): SparseCategoricalCrossEntropy[Float] = {

    /* Whether to accept log-probabilities or probabilities */
    val logProbAsInput = getAsBoolean(params, "logProbAsInput", default = false)
   ...
   * Squared Hinge
   */
  private def config2SquaredHinge(params:Config): SquaredHinge[Float] = {
