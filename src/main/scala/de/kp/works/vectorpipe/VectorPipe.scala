...
import de.kp.works.vectorpipe.vectortile._
...
object VectorPipe {
  ...
  object Options {
    ...
    def forZoom(zoom: Int): Options = Options(zoom, None, LatLng)

    def forZoomRange(minZoom: Int, maxZoom: Int): Options = Options(maxZoom, Some(minZoom), LatLng)

    def forAllZoomsFrom(zoom: Int): Options = Options(zoom, Some(0), LatLng)

    def forAllZoomsWithSrcProjection(zoom: Int, crs: CRS): Options = Options(zoom, Some(0), crs)
  }

  /**
    * Run VectorPipe.
    *
    * @param input       Input OSM geometries DataFrame (See: [[de.kp.works.vectorpipe.OSM#toGeometry]])
